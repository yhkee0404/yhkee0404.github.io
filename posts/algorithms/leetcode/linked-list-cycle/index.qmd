...
프로그래머스 문제 [아날로그 시계](https://school.programmers.co.kr/learn/courses/30/lessons/250135)는 시침, 분침, 초침이 겹치는 횟수를 세는 문제이다. 이 문제가 요구하는 것은 Two Pointers 탐색이 아니라 앞서 얘기한 Cartesian Product를 시간순으로 탐색할 때의 지역적 재방문 횟수이다: $O(n^k) \ne O(kn)$. 왜냐하면 아날로그 시계는 가령 초 단위로 $12 * 60 * 60$개 상태를 탐색하지 Two Pointers처럼 $12 + 60 + 60$개를 탐색하지는 않기 때문이다.

몇 시간, 몇 분, 몇 초가 지났는지라는 완전히 지역적인 중복 방문 횟수는 쉽게 셀 수 있지만 문제는 상대적으로 전역적인 중복 방문 횟수를 요구한다는 것이다: 아날로그 시계라는 자료 구조를 공유하는 Iterator들이 같은 원소를 중복 방문하는 횟수; 엄밀히 전역적인 중복 방문은 없다. 더 큰 문제는 중복이 정수가 아닌 실수 공간에서 발생한다는 사실이다. 마지막으로 [14](#cb5-14)번째 줄과 [27-28](#cb5-28)번째 줄에서 중복 횟수를  
