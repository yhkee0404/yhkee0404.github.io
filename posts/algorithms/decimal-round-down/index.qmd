...
소수점 이하 무한한 자릿수가 모두 필요할 일은 거의 없다. 가령 소수점 이하 `n`번째 자리에서 최소 오차를 예상할 수 있다면 `n + 1`번째 자리에 작은 수를 더하거나 빼 주면 된다.

```{python}
_EPSILON = 1e-2
four = (7 - 10 / 6 * 3 + 1e-2) * 2
print(four)
print(int(four))
```
...
four = (Decimal('7') - Decimal('10') / Decimal('6') * Decimal('3')) * 2 + Decimal('1e-2')
print(four)
print(int(four))
print(four.quantize(Decimal('0'), ROUND_DOWN))
```