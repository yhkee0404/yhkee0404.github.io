...
7. Scale의 기능
  ...
  Softmax의 입력을 1보다 큰 Scale로 나누는 것은 Softmax Temperature^[see @boonstra2024prompt]로서 예측 불확실성을 높인다.
  ...
  다만 모델 출력의 Softmax 직전과, 출력을 다시 Decoder에 입력할 때, 그리고 Encoder의 입력을 만들 때의 세 경우에는 모두 같은 가중치 행렬을 공유하며 선형 변환을 수행하면서도, 이번에는 Softmax 입력의 Scale을 줄이는 대신에 반대로 후자의 두 경우에서 Scale을 늘린다 [see @vaswani2017attentionneed, Section 3.4, p. 5].
